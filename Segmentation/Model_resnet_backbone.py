...
conv6=tf.keras.layers.Conv2D(filters=1024,
                            ...
                            padding='same')(resnet.get_layer("conv5_block3_out").output)
relu1=tf.keras.layers.ELU()(conv6)
do1=tf.keras.layers.Dropout(0.2)(relu1)
upx=tf.keras.layers.Conv2DTranspose(1024, (2, 2), strides=(2, 2), padding='same')(do1)

#Upsampling block 1
conc1=tf.keras.layers.Concatenate(axis=-1)([upx,resnet.get_layer("conv4_block6_out").output])

conv7=tf.keras.layers.Conv2D(filters=512,
                            kernel_size=(3, 3),kernel_initializer='he_normal',
                            padding='same')(conc1)
relu2=tf.keras.layers.ELU()(conv7)
do2=tf.keras.layers.Dropout(0.2)(relu2)
up1=tf.keras.layers.Conv2DTranspose(512, (2, 2), strides=(2, 2), padding='same')(do2)

#Upsampling block 2
conc2=tf.keras.layers.Concatenate(-1)([up1,resnet.get_layer("conv3_block4_out").output])

conv8=tf.keras.layers.Conv2D(filters=256,
                            kernel_size=(3, 3),kernel_initializer='he_normal',
                            padding='same')(conc2)
relu3=tf.keras.layers.ELU()(conv8)
do3=tf.keras.layers.Dropout(0.2)(relu3)
up2=tf.keras.layers.Conv2DTranspose(256, (2, 2), strides=(2, 2), padding='same')(do3)

#Upsampling block 3
conc3=tf.keras.layers.Concatenate(-1)([up2,resnet.get_layer("conv2_block3_out").output])

conv9=tf.keras.layers.Conv2D(filters=128,
                            kernel_size=(3, 3),kernel_initializer='he_normal',
                            padding='same')(conc3)

relu4=tf.keras.layers.ELU()(conv9)
do4=tf.keras.layers.Dropout(0.2)(relu4)
up3=tf.keras.layers.Conv2DTranspose(128, (2, 2), strides=(2, 2), padding='same')(do4)

#Upsampling block 4
conc4=tf.keras.layers.Concatenate(-1)([up3,resnet.get_layer("conv1_relu").output])

conv10=tf.keras.layers.Conv2D(filters=64,
                            kernel_size=(3, 3),kernel_initializer='he_normal',
                            padding='same')(conc4)

relu5=tf.keras.layers.ELU()(conv10)
do5=tf.keras.layers.Dropout(0.1)(relu5)
up4=tf.keras.layers.Conv2DTranspose(64, (2, 2), strides=(2, 2), padding='same')(do5)
...
conv11=tf.keras.layers.Conv2D(filters=64,
                            kernel_size=(3, 3),kernel_initializer='he_normal',
                            padding='same')(up4)
relu6=tf.keras.layers.ELU()(conv11)
...
with open("model_resnet_backbone_dout.json", "w") as json_file:
...
loss = weighted_bce_dice_loss_boundary
# learning rate
lr = 1e-5
...
# Compile Model 
model.load_weights(os.path.join(cwd,"experiments_dir\\model_resnet_UnetDec17_18-14-14\\ckpts\\mymodel_2.h5"))
...
lrr = ReduceLROnPlateau(monitor='competitionMetric', factor=0.1, patience=1, min_lr=0.000001, verbose=1)
